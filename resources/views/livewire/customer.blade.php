                        ...
                        <div class="overflow-x-auto">
                            @if(count($customers) > 0)
                                <table class="table scrollable">
                                    ...
                                    @foreach($customers as $key => $customer)
                                        <tr id="tr_{{$customer->id}}">
                                            ...
                                            <td class="border-b whitespace-no-wrap">{{ $customer->address }}</td>
                                            <td class="border-b whitespace-no-wrap">{{ $customer->postal_code }}</td>
                                            <td class="border-b whitespace-no-wrap">{{ optional($customer->country)->title ?? 'N/A' }}</td>
                                            <td class="border-b whitespace-no-wrap">{{ optional($customer->city)->title ?? 'N/A' }}</td>
                                            <td class="border-b whitespace-no-wrap">{{ optional($customer->area)->title ?? 'N/A' }}</td>
                                ...
                                @if($customers->total() > $customers->perPage())
                                    {{ $customers->links() }}
                                @else
                                    Showing {{ $customers->firstItem() }} to {{ $customers->lastItem() }} out of {{ $customers->total() }}
                                    results
                                @endif
                            @else
